...
- **WebSite Schema:** Homepage site identity
- **Service Schema:** Rich results for service offerings
- **FAQPage Schema:** FAQ snippets in Google

### Technical SEO
- **XML Sitemap:** Generated by the app router (`app/sitemap.ts`) with www URLs and per-page priorities
- **Robots.txt:** Environment-aware (production: allow with exclusions, staging: block all)
- **AI crawler access:** GPTBot, ClaudeBot, PerplexityBot, Google-Extended and other AI crawlers explicitly allowed (GEO)
- **GEO files:** `llms.txt` and `llms-full.txt` for generative engine discovery and citation
...
- **Sitemap:** `app/sitemap.ts` - Sitemap generation rules
- **Social images:** `public/og-images/*.png` (regenerate with `scripts/generate-og-images.sh`)
