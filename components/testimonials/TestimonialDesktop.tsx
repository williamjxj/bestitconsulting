/**
 * Testimonial desktop component
 * Provides desktop-optimized testimonial display and management
 */
...
interface TestimonialDesktopProps {
  testimonials: Array<{
    id: string
    name: string
    role: string
  ...
  showMedia?: boolean
  showSocial?: boolean
  onDesktopAction?: (action: string, data: any) => void
}

export const TestimonialDesktop: React.FC<TestimonialDesktopProps> = ({
  testimonials,
  className = '',
  showSidebar = true,
  showToolbar = true,
  ...
  const [isPlaying, setIsPlaying] = useState(desktopSettings.autoPlay)
  ...
  const handleTestimonialAction = useCallback(
    async (testimonialId: string, action: string, data?: any) => {
      ...
      try {
        ...
        onDesktopAction?.(action, { testimonialId, ...data })
      } catch (error) {
  ...
  const handleBulkAction = useCallback(
    ...
    [selectedTestimonials, onDesktopAction]
  )
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPlaying || !desktopSettings.autoPlay) return

    const interval = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % filteredTestimonials.length)
    }, desktopSettings.autoPlayInterval)
  }, [
    isPlaying,
    desktopSettings.autoPlay,
    desktopSettings.autoPlayInterval,
    filteredTestimonials.length,
  ])
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredTestimonials.map((testimonial, index) => (
                      ...
                      >
                        ...
                        {/* Date */}
                        {desktopSettings.showDates && testimonial.date && (
