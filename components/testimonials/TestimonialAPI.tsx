/**
 * Testimonial API component
 * Provides API interface for testimonial management
 */
...
interface TestimonialAPIProps {
  ...
  showAdmin?: boolean
  onAPIAction?: (action: string, data: any) => void
}

export const TestimonialAPI: React.FC<TestimonialAPIProps> = ({
  testimonials,
  className = '',
  showHeader = true,
  showSidebar = true,
  ...
  const [isPlaying, setIsPlaying] = useState(apiSettings.autoPlay)
  ...
  const handleTestimonialAction = useCallback(
    async (testimonialId: string, action: string, data?: any) => {
      ...
      try {
        ...
        onAPIAction?.(action, { testimonialId, ...data })
      } catch (error) {
  ...
  const handleBulkAction = useCallback(
    async (action: string, data?: any) => {
      ...
      try {
        ...
        onAPIAction?.(action, { testimonialIds: selectedTestimonials, ...data })
    ...
    [selectedTestimonials, onAPIAction]
  )
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPlaying || !apiSettings.autoPlay) return

    const interval = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % filteredTestimonials.length)
    }, apiSettings.autoPlayInterval)
  }, [
    isPlaying,
    apiSettings.autoPlay,
    apiSettings.autoPlayInterval,
    filteredTestimonials.length,
  ])
          ...
          <motion.div variants={itemVariants}>
            <div className='flex items-center justify-between p-4 bg-background border-b'>
              <div className='flex items-center gap-4'>
                <h1 className='text-2xl font-bold'>Testimonial API</h1>
              ...
              <div className='flex items-center gap-4'>
                <div className='flex items-center gap-2'>
                  <Button
                    size='sm'
                    variant={
                      apiSettings.viewMode === 'list' ? 'default' : 'outline'
                    }
                  ...
                  <Button
                    size='sm'
                    variant={
                      apiSettings.viewMode === 'grid' ? 'default' : 'outline'
                    }
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredTestimonials.map((testimonial, index) => (
                      ...
                      >
                        ...
                        {/* Date */}
                        {apiSettings.showDates && testimonial.date && (
