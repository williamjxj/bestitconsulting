/**
 * Testimonial webhook component
 * Provides webhook interface for testimonial management
 */
...
interface TestimonialWebhookProps {
  testimonials: Array<{
    id: string
    name: string
    role: string
  ...
  showAdmin?: boolean
  onWebhookAction?: (action: string, data: any) => void
}

export const TestimonialWebhook: React.FC<TestimonialWebhookProps> = ({
  testimonials,
  className = '',
  showHeader = true,
  showSidebar = true,
  ...
  const [isPlaying, setIsPlaying] = useState(webhookSettings.autoPlay)
  ...
  const handleTestimonialAction = useCallback(
    async (testimonialId: string, action: string, data?: any) => {
      ...
      try {
        ...
        onWebhookAction?.(action, { testimonialId, ...data })
      } catch (error) {
  ...
  const handleBulkAction = useCallback(
    ...
    [selectedTestimonials, onWebhookAction]
  )
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPlaying || !webhookSettings.autoPlay) return

    const interval = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % filteredTestimonials.length)
    }, webhookSettings.autoPlayInterval)
  }, [
    isPlaying,
    webhookSettings.autoPlay,
    webhookSettings.autoPlayInterval,
    filteredTestimonials.length,
  ])
          ...
          <motion.div variants={itemVariants}>
            <div className='flex items-center justify-between p-4 bg-background border-b'>
              <div className='flex items-center gap-4'>
                <h1 className='text-2xl font-bold'>Testimonial Webhook</h1>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredTestimonials.map((testimonial, index) => (
                      ...
                      >
                        ...
                        {/* Date */}
                        {webhookSettings.showDates && testimonial.date && (
...
export default TestimonialWebhook
