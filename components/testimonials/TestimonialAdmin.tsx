/**
 * Testimonial admin component
 * Provides admin interface for testimonial management
 */
...
export const TestimonialAdmin: React.FC<TestimonialAdminProps> = ({
  testimonials,
  className = '',
  showHeader = true,
  showSidebar = true,
  ...
  const [isPlaying, setIsPlaying] = useState(adminSettings.autoPlay)
  ...
  const handleTestimonialAction = useCallback(
    async (testimonialId: string, action: string, data?: any) => {
      ...
      try {
        ...
        onAdminAction?.(action, { testimonialId, ...data })
      } catch (error) {
  ...
  const handleBulkAction = useCallback(
    ...
    [selectedTestimonials, onAdminAction]
  )
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPlaying || !adminSettings.autoPlay) return

    const interval = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % filteredTestimonials.length)
    }, adminSettings.autoPlayInterval)
  }, [
    isPlaying,
    adminSettings.autoPlay,
    adminSettings.autoPlayInterval,
    filteredTestimonials.length,
  ])
          ...
          <motion.div variants={itemVariants}>
            <div className='flex items-center justify-between p-4 bg-background border-b'>
              <div className='flex items-center gap-4'>
                <h1 className='text-2xl font-bold'>Testimonial Admin</h1>
              ...
              <div className='flex items-center gap-4'>
                <div className='flex items-center gap-2'>
                  <Button
                    size='sm'
                    variant={
                      adminSettings.viewMode === 'list' ? 'default' : 'outline'
                    }
                  ...
                  <Button
                    size='sm'
                    variant={
                      adminSettings.viewMode === 'grid' ? 'default' : 'outline'
                    }
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredTestimonials.map((testimonial, index) => (
                      ...
                      >
                        ...
                        {/* Date */}
                        {adminSettings.showDates && testimonial.date && (
