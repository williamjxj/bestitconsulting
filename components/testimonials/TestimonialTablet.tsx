/**
 * Testimonial tablet component
 * Provides tablet-optimized testimonial display and management
 */
...
interface TestimonialTabletProps {
  testimonials: Array<{
    id: string
    name: string
    role: string
  ...
  showMedia?: boolean
  showSocial?: boolean
  onTabletAction?: (action: string, data: any) => void
}

export const TestimonialTablet: React.FC<TestimonialTabletProps> = ({
  testimonials,
  className = '',
  showSidebar = true,
  showToolbar = true,
  ...
  const [isPlaying, setIsPlaying] = useState(tabletSettings.autoPlay)
  ...
  const handleTestimonialAction = useCallback(
    async (testimonialId: string, action: string, data?: any) => {
      ...
      try {
        ...
        onTabletAction?.(action, { testimonialId, ...data })
      } catch (error) {
  ...
  const handleBulkAction = useCallback(
    ...
    [selectedTestimonials, onTabletAction]
  )
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPlaying || !tabletSettings.autoPlay) return

    const interval = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % filteredTestimonials.length)
    }, tabletSettings.autoPlayInterval)
  }, [
    isPlaying,
    tabletSettings.autoPlay,
    tabletSettings.autoPlayInterval,
    filteredTestimonials.length,
  ])
              ...
              <div className='flex items-center gap-4'>
                <div className='flex items-center gap-2'>
                  <Button
                    size='sm'
                    variant={
                      tabletSettings.viewMode === 'list' ? 'default' : 'outline'
                    }
                  ...
                  <Button
                    size='sm'
                    variant={
                      tabletSettings.viewMode === 'grid' ? 'default' : 'outline'
                    }
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredTestimonials.map((testimonial, index) => (
                      ...
                      >
                        ...
                        {/* Date */}
                        {tabletSettings.showDates && testimonial.date && (
...
export default TestimonialTablet
